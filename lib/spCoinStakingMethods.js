...
class SpCoinStakingMethods {
  ...
  testStakingRewards = async(lastUpdateTime, testUpdateTime, interestRate, quantity) => {
    ...
    let stakingRewards = await _spCoinContractSigned.testStakingRewards(lastUpdateTime, testUpdateTime, interestRate, quantity);
  ...
  getStakingRewards = async(lastUpdateTime,  interestRate,  quantity) => {
    // spCoinLogger.logFunctionHeader("getStakingRewards(lastUpdateTime,  interestRate,  quantity)");
    let stakingRewards = await _spCoinContractSigned.getStakingRewards(lastUpdateTime,  interestRate,  quantity);
  ...
  getTimeMultiplier = async(_timeRateMultiplier) => {
    ...
    let timeRateMultiplier = await _spCoinContractSigned.getTimeMultiplier(_timeRateMultiplier);
  ...
  getAccountTimeInSecondeSinceUpdate = async(_tokenLastUpdate) => {
    let timeInSecondeSinceUpdate = await _spCoinContractSigned.getAccountTimeInSecondeSinceUpdate(_tokenLastUpdate);
  ...
  depositSponsorStakingRewards = async (
    ...
    await _spCoinContractSigned.depositStakingRewards (
      SPONSOR,
      _sponsorAccount,
      _recipientAccount,
      _recipientRate ,
